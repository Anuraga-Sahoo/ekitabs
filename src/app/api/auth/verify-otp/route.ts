...
export async function POST(request: NextRequest) {
  try {
    ...
    // Final check if a verified user already exists (e.g., race condition)
    const existingVerifiedUser = await usersCollection.findOne({ email: userDetails.email, isVerified: true });
    if (existingVerifiedUser) {
      return NextResponse.json({ message: "User with this email is already verified." }, { status: 409 });
    }
    
    // Upsert logic: if user exists but not verified, update them. Otherwise, insert new.
    // This handles cases where a user might try to sign up, get an OTP, but not complete it,
    // then try again. We'd want to update their record rather than fail.
    const result = await usersCollection.updateOne(
      { email: userDetails.email },
      {
        $set: {
          ...
          isVerified: true,
          createdAt: new Date(), // Or update if user already exists, set on insert if new
        },
        $setOnInsert: { // Only set these if it's a new document
            email: userDetails.email, 
        }
      },
      { upsert: true }
    ...
    if (result.upsertedId || result.modifiedCount > 0 || result.matchedCount > 0) {
        ...
        // This case should ideally not be reached if upsert is working correctly
