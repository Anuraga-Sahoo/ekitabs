...
const GenerateMockTestInputSchema = z.object({
  numberOfQuestions: z
    .number()
    .default(50) // Defaulting to 50
    .describe('The total number of MCQ questions to generate for the mock test. This MUST be 50 for the current configuration.'),
});
...
export async function generateMockTest(input: GenerateMockTestInput): Promise<GenerateMockTestOutput> {
  // Ensure the input adheres to the 50 total questions.
  if (input.numberOfQuestions !== 50) {
    throw new Error("Mock test generation is configured for exactly 50 questions (12 Physics, 13 Chemistry, 25 Biology).");
  }
...
1.  **Physics Questions (First 12 MCQs):**
...
2.  **Chemistry Questions (Next 13 MCQs, from question 13 to 25):**
...
3.  **Biology Questions (Final 25 MCQs, from question 26 to 50):**
...
*   The total number of questions in the "questions" array MUST be exactly {{numberOfQuestions}}. This means 12 Physics, then 13 Chemistry, then 25 Biology, in that order.
